package dk.kultur.historiejagtenfyn.ui.Views.beyondar;
...
public class ARRenderer extends com.beyondar.android.opengl.renderer.ARRenderer implements GLSurfaceView.Renderer, BeyondarSensorListener,
	 ...
	 */
	protected void convertGPStoPoint3(GeoObject geoObject, Point3 out, double dst) {
		float x, z, y;
		...
		if (mMaxDistanceSizePoints > 0 || mMinDistanceSizePoints > 0) {
			...
			if (mMinDistanceSizePoints > 0 && totalDst < mMinDistanceSizePoints) {
               // totalDst = mMinDistanceSizePoints;
		...
		//float ratio = (float) (totalDst * 1 / (mMaxDistanceSizePoints / mDistanceFactor));
		//ratio -= 4;
		out.x = x;
		out.y = y;
		out.z = z -3;
	...
	protected void renderBeyondarObject(GL10 gl, BeyondarObject beyondarObject, Texture defaultTexture,
		...
		if (beyondarObject instanceof GeoObject) {
			dst = ((GeoObject) beyondarObject).calculateDistanceMeters(mWorld.getLongitude(),
					mWorld.getLatitude());
			convertGPStoPoint3((GeoObject) beyondarObject, beyondarObject.getPosition(), dst);
		} else {
	...
	public void loadBeyondarObjectTexture(GL10 gl, BeyondarObject geoObject) {
		...
		if (texture == null) {
			Bitmap btm = null;
			if (mWorld.getBitmapCache().isImageLoaded(geoObject.getImageUri())) {
				btm = mWorld.getBitmapCache().getBitmap(geoObject.getImageUri());
			} else {
				try {
					btm = BitmapFactory.decodeStream(FileUtils.getFileStream(mContext, geoObject.getImageUri()));
					btm.setDensity(DisplayMetrics.DENSITY_XHIGH);
				} catch (IOException e) {
					e.printStackTrace();
				}
